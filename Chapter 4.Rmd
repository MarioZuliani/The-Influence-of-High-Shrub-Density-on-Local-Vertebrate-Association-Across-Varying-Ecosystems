...
plot3 + scale_fill_manual(values = c("#009900", "#0066cc"))
```
...
ggplot(p02, aes(Axis.1, Axis.2, group = microsite)) +
  geom_point(aes(color = microsite)) +
  geom_text(aes(label=plot), hjust = 0, vjust = 0, check_overlap = TRUE, nudge_x = 0.01)+
  scale_color_brewer(palette = "Set1") +
  labs(color = "", subtitle = "labels denote plot identity")
...
```{r}
shapiro.test(final_data$shrub_density)
```

```{r}
### Abundance
model1 <- glm(animals~shrub_density*site*mean_temp*year + aridity  + mean_humidity, family = "poisson", data = final_data) 
model1
anova_results <- aov(model1, test = "Chisq")
...
### Richness Stats
model2 <- glm(richness~shrub_density*site*mean_temp + year + aridity  + mean_humidity, family = "poisson", data = final_data)
model2
anova_results2 <- aov(model2, test = "Chisq")
summary(anova_results2)
e3 <- emmeans(model2, pairwise~site|year)
e3  ### Need to double check emmeans it does not match figure
```
```{r}
### Evenness
model3 <- glm(Average_Evenness~shrub_density*site*mean_temp + year + aridity  + mean_humidity, family = "poisson", data = final_data)
model3
...
richness <- ggplot(final_data, aes(shrub_density, richness, color = site)) +
  ...
  labs(x = "Shrub Density (Individuals per 20m radius)", y = "Species Richness")
richness
...
Evenness <- ggplot(final_data, aes(shrub_density, Average_Evenness, color = site)) +
  ...
  labs(x = "Shrub Density (Individuals per 20m radius)", y = "Evenness")
Evenness
```
### Stats for Average Temperature
```{r}
### Abundance and temperature
model4 <- glm(animals~mean_temp*site*year + aridity  + mean_humidity, family = "poisson", data = final_data) 
model4
anova_results4 <- aov(model4, test = "Chisq")
summary(anova_results4)
e5 <- emmeans(model4, pairwise~site)
e5
```
```{r}
### Richness and temperature
model5 <- glm(richness~mean_temp*site*year + aridity  + mean_humidity, family = "poisson", data = final_data) 
model5
anova_results5 <- aov(model5, test = "Chisq")
summary(anova_results5)
e6 <- emmeans(model5, pairwise~site)
e6
```
```{r}
### Evenness and temperature
model6 <- glm(richness~mean_temp*site*year + aridity  + mean_humidity, family = "poisson", data = final_data) 
model6
anova_results6 <- aov(model6, test = "Chisq")
summary(anova_results6)
e7 <- emmeans(model6, pairwise~site)
e7
```
...
abundance_temp <- ggplot(final_data, aes(mean_temp, animals)) +
  ...
  scale_color_brewer(palette = "Set1") + theme_classic() + 
  geom_smooth(method = lm, se = TRUE) + theme(axis.title.x = element_blank()) + labs(tag = "A")+
  labs(x = "Average Temperature (C)", y = "Animal abundance")
abundance_temp
...
richness_temp <- ggplot(final_data, aes(mean_temp, richness)) +
  ...
  scale_color_brewer(palette = "Set1") + theme_classic() + 
  geom_smooth(method = lm, se = TRUE) + theme(axis.title.x = element_blank()) + labs(tag = "B")+
  labs(x = "Average Temperature (C)", y = "Species Richness")
richness_temp
...
evenness_temp <- ggplot(final_data, aes(mean_temp, Average_Evenness)) +
  ...
  scale_color_brewer(palette = "Set1") + theme_classic() + theme(legend.position = "none") +
  geom_smooth(method = lm, se = TRUE) +  labs(tag = "C")+
  labs(x = "Average Temperature (C)", y = "Evenness")
evenness_temp
```
